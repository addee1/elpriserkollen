...
function applyFees(data: HourlyPrice[], fees: FeesConfig): HourlyPrice[] {
  ...
  return data.map((h) => {
    let price =
      h.price +
      (fees.rorligaKostnader + fees.fastaPaslag + fees.eloverforing + fees.energiskatt) / 100;
  ...
  return (
    <div className="app">
      ...
      <main className="main-content">
        <AreaDaySelector area={area} day={day} onAreaChange={setArea} onDayChange={setDay} />
        ...
        {error && (
          <div className="error-state">
            Kunde inte hämta priser. {day === 'tomorrow' ? 'Morgondagens priser publiceras vanligtvis runt kl 13:00.' : 'Försök igen senare.'}
          </div>
        )}

        {hourlyData && (
          <>
            <PriceOverview data={hourlyData} unit={unit} onUnitChange={setUnit} />
